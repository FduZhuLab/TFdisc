...
def TF_grn(G_data,
    ...
    if tf is None:
        ...
        if not set(gene).intersection(set(tf)):
            raise ValueError('Intersection of gene and tf names is empty.')
    start = time.time()
    local_cluster = LocalCluster(n_workers=n_core,threads_per_worker=1)
    custom_client = Client(local_cluster)
    
    if method == "grnboost2":
        adjancencies = grnboost2(expression_data=G_data, tf_names=tf, verbose=verbose,client_or_address=custom_client)
    if method == "genie3":
        adjancencies = genie3(expression_data=G_data, tf_names=tf, verbose=verbose,client_or_address=custom_client)
